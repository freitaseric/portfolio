...
export const GET: APIRoute = ({ url }) => {
  ...
  const svg = `
    ...
    height - 100
  }" r="80" fill="${accentColor}" opacity="0.1"/>
      <circle cx="${
        width - 200
      }" cy="150" r="30" fill="${primaryColor}" opacity="0.05"/>
      
      <!-- Linha decorativa -->
      <line x1="0" y1="${height * 0.7}" x2="${width}" y2="${
    height * 0.7
  }" stroke="url(#accentGradient)" stroke-width="3" opacity="0.3"/>
      
      <!-- Logo/Nome principal -->
      <text x="${
        width / 2
      }" y="200" font-family="Inter, Arial, sans-serif" font-size="72" font-weight="700" text-anchor="middle" fill="${textColor}">
        Eric Freitas
      </text>
      ...
      <text x="${
        width / 2
      }" y="320" font-family="Inter, Arial, sans-serif" font-size="48" font-weight="600" text-anchor="middle" fill="${textColor}">
        ${title}
      </text>
      ...
      <text x="${
        width / 2
      }" y="400" font-family="Inter, Arial, sans-serif" font-size="28" font-weight="400" text-anchor="middle" fill="${textColor}" opacity="0.8">
        ${description}
      </text>
      ...
      <rect x="${
        width / 2 - 80
      }" y="450" width="160" height="40" rx="20" fill="url(#accentGradient)" opacity="0.9"/>
      <text x="${
        width / 2
      }" y="475" font-family="Inter, Arial, sans-serif" font-size="18" font-weight="500" text-anchor="middle" fill="${textColor}">
        ${locale}
      </text>
      ...
      <text x="${width / 2}" y="${
    height - 50
  }" font-family="Inter, Arial, sans-serif" font-size="20" font-weight="400" text-anchor="middle" fill="${textColor}" opacity="0.6">
        freitaseric.com
      </text>
    </svg>
  `;
